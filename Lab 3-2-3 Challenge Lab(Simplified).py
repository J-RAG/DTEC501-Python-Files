...
if len(user_name) == NO_INPUT:
    ...
    if name_validator.isalpha() and len(name_validator) >= NAME_LENGTH_REQUIREMENT:
        ...
        if pin_number.isdigit() and len(pin_number) == PIN_NUMBER_LENGTH_REQUIREMENT:
            ...
            #This is the the simplified version of: not(password.isalpha()) and not(password.isdigit()))
            if (not (password.isalpha() or password.isdigit()) and len(password) == PASSWORD_LENGTH_REQUIREMENT:
