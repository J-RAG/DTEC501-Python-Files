# Lab 5-1-2 Challenge Lab question 7
...
while non_exit_input:
	...
	# loop sequence in normal iterations 
        for amount in range(1, int(num_asterisks)): # exclusive stop to prevent duplicate in next loop
            print(f"{SPACE_SEPARATOR * (int(num_asterisks) - amount)}"\
                  f"{((ASTERISK_TO_PRINT + SPACE_SEPARATOR) * amount).strip()}")
